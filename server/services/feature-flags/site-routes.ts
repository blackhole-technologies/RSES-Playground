...
router.get("/:key", async (req, res) => {
  try {
    ...
    log.error({ err, key: req.params.key }, "Failed to get site flag");
...
router.put("/:key", requireAuth, async (req, res) => {
  try {
    ...
    const flag = await service.updateFlag(req.params.key, updates);
    ...
    log.error({ err, key: req.params.key }, "Failed to update site flag");
...
router.delete("/:key", requireAuth, async (req, res) => {
  try {
    ...
    log.info(
      { key: req.params.key, siteId: tryGetSiteContext()?.siteId },
    ...
    log.error({ err, key: req.params.key }, "Failed to delete site flag");
 ...
 */
router.post("/:key/enable", requireAuth, async (req, res) => {
  try {
    const service = getSiteScopedFeatureFlagsService();
    const success = await service.enableFlag(req.params.key);

    if (!success) {
      return res.status(404).json({ message: "Feature flag not found" });
    }

    res.json({ message: "Flag enabled", key: req.params.key });
  } catch (err) {
    if (err instanceof SiteContextError) {
      return res.status(400).json({ error: err.message, code: err.code });
    }
    log.error({ err, key: req.params.key }, "Failed to enable site flag");
 ...
 */
router.post("/:key/disable", requireAuth, async (req, res) => {
  try {
    const service = getSiteScopedFeatureFlagsService();
    const success = await service.disableFlag(req.params.key);

    if (!success) {
      return res.status(404).json({ message: "Feature flag not found" });
    }

    res.json({ message: "Flag disabled", key: req.params.key });
  } catch (err) {
    if (err instanceof SiteContextError) {
      return res.status(400).json({ error: err.message, code: err.code });
    }
    log.error({ err, key: req.params.key }, "Failed to disable site flag");
 ...
 */
router.get("/:key/user-overrides", async (req, res) => {
  try {
    const service = getSiteScopedFeatureFlagsService();
 ...
 */
router.put("/:key/user-overrides/:userId", requireAuth, async (req, res) => {
  try {
    const service = getSiteScopedFeatureFlagsService();
    ...
    log.error({ err, key: req.params.key }, "Failed to set user override");
...
router.get("/:key/stats", async (req, res) => {
  try {
    ...
    log.error({ err, key: req.params.key }, "Failed to get site stats");
...
router.get("/:key/history", async (req, res) => {
  try {
    ...
    log.error({ err, key: req.params.key }, "Failed to get site history");
