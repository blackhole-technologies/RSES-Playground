 ...
 *
 * The original file had **no authentication at all** on any route —
 * every handler was exposed if the router were mounted. The migration
 * wraps every handler in `protect("watcher:<action>", ...)` using
 * forward-referenced permission keys. `rbacService.hasPermission`
 * fails closed on unknown keys, so non-admins are denied until the
 * permissions are seeded in the RBAC migration.
 *
 * The previous `requireIntelligentWatcher` middleware (which resolved
 * the service handle and attached it to `req.intelligentWatcher`) has
 * been inlined via the local `resolveWatcher(res)` helper, so the
 * marker remains the outermost call at every route site.
 *
 * TODO: Define `watcher:{read,manage}` permissions in the RBAC seed
 * migration once this file is actually mounted in the app.
 */
...
/**
 * Resolve the intelligent watcher service, writing a 503 response and
 * returning null on failure. This replaces the previous
 * `requireIntelligentWatcher` middleware by inlining the resolution at
 * the start of every handler — the marker wrapper must be the outermost
 * callable in the route signature, so standalone middleware doesn't fit.
 */
function resolveWatcher(res: Response): IntelligentFileWatcherService | null {
  try {
    return getIntelligentFileWatcher();
  } catch {
    res.status(503).json({
...
router.get(
  "/status",
  protect("watcher:read", async (req, res) => {
    try {
      const watcher = resolveWatcher(res);
      if (!watcher) return;
...
router.get(
  ...
  protect("watcher:read", async (req, res) => {
    try {
      ...
      const change = changes.find((c) => c.id === String(req.params.id));
...
router.put(
  "/degradation",
  protect("watcher:manage", async (req, res) => {
    try {
...
router.post(
  ...
  protect("watcher:read", async (req, res) => {
    try {
      ...
      // Mock response — full implementation would perform actual analysis.
...
router.get(
  ...
  protect("watcher:read", async (req, res) => {
    try {
      const _watcher = resolveWatcher(res);
      if (!_watcher) return;
...
router.get(
  ...
  protect("watcher:read", async (req, res) => {
    try {
      ...
      const changes = watcher.getChangeHistory();
      const change = changes.find((c) => c.id === String(req.params.changeId));
