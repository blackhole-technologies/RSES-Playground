...
/**
 * Hook for persisting state to sessionStorage (cleared on tab close).
 *
...
export function useSessionStorage<T>(
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    ...
    try {
      ...
      console.warn(`[useSessionStorage] Error reading ${key}:`, error);
      return initialValue;
    }
  });
  ...
  const setValue = useCallback(
    (value: T | ((prev: T) => T)) => {
      try {
        ...
        console.warn(`[useSessionStorage] Error writing ${key}:`, error);
      }
    },
    [key, storedValue]
  );
  ...
  const removeValue = useCallback(() => {
    try {
      ...
      console.warn(`[useSessionStorage] Error removing ${key}:`, error);
    }
  }, [key, initialValue]);
