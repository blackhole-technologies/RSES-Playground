 ...
 *
 * Every handler is wrapped in `protect("sites:...", handler)`. The prior
 * `router.use(requireAuth); router.use(requireAdmin)` lines are removed
 * because `protect()` already enforces authentication (and admins bypass
 * the permission check via `user.isAdmin` in rbac-protect.ts).
 */
...
router.get(
  "/",
  protect("sites:read", async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post(
  "/bulk-action",
  protect("sites:manage", async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.put(
  "/:siteId",
  protect("sites:update", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const siteId = String(req.params.siteId);
